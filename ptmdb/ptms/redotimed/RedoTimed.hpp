...
template<typename T> inline void persist<T>::pstore(T newVal) {
        ...
        if (offset != 0 && ADDR_IS_IN_MAIN(valaddr)) {
        	...
        	sameCL = gRedo.add_to_log((RedoTimed::State*)tlocal.st, valaddr, oldval, (uint64_t)newVal);
           ...
           if(!sameCL) DEFER_PWB(ADDR2CL(valaddr + offset));
            ...
            if (ADDR_IS_IN_REGION(valaddr)) {
            	sameCL = gRedo.add_to_log((RedoTimed::State*)tlocal.st, valaddr - offset, (uint64_t)val, (uint64_t)newVal);
