 ...
 */
#ifndef _PERSISTENT_BENCHMARK_LATENCY_Q_H_
#define _PERSISTENT_BENCHMARK_LATENCY_Q_H_
...
private:
    ...
    // Latency constants
    static const long long kLatencyMeasures =     20000000LL;    // We measure 20M iterations divided among the different threads
    ...
    static const long long kLatencyIterations =         200;     // We do this many iterations of 10K enqueues and dequeues until we get kLatencyMeasures
...
public:
    PBenchmarkLatencyQueues(int numThreads) {
...
#endif
    void latencyBurstBenchmark(std::string& className) {
...
#elif defined (USE_FRIEDMAN) || defined (USE_NORMALIZED_OPT)
            queue = new Q();
#else
        ...
        auto latency_lambda = [this,&startEnq,&startDeq,&barrier,&queue](nanoseconds* enqDelays, nanoseconds* deqDelays, const int tid) {
            ...
            for (int iter=0; iter < kLatencyIterations+kLatencyWarmupIterations; iter++) {
                ...
                for (long long i = 0; i < kLatencyBurst/numThreads; i++) {
                    auto startBeats = steady_clock::now();
                    if (queue->dequeue(tid) == queue->EMPTY) cout << "ERROR: dequeued nullptr in i=" << i << "\n";
                    auto stopBeats = steady_clock::now();
