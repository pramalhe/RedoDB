...
/*
 * <h1> Redo Persistent Transactional Memory </h1>
...
class Redo {
    ...
    static const int IDX_BITS = 12;  // 12 => STATESIZE up to 4096
...
private:
    ...
    // Copy a range to a persistent memory region using memcpy+pwbs.
...
public:
    ...
    Redo() : dommap{true},maxThreads{MAX_THREADS}{
        ...
        for (int i=0; i < RINGSIZE; i++) ring[i].store(0, std::memory_order_relaxed);
...
template<typename T> inline void persist<T>::pstore(T newVal) {
    ...
    if (offset != 0 && ADDR_IS_IN_MAIN(valaddr)) {
        ...
        bool sameCL = gRedo.add_to_log(valaddr, oldval, (uint64_t)newVal);
        ...
        bool sameCL = gRedo.add_to_log(valaddr - offset, (uint64_t)val, (uint64_t)newVal);
